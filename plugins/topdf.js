...
/**
 * Mengunggah buffer file ke host sementara yang mendukung semua jenis file.
...
async function uploadFile(buffer, filename) {
    ...
    if (data.status !== 'success') {
        throw new Error('Gagal mengunggah file ke host sementara.');
    }
    // URL dari tmpfiles perlu diubah sedikit untuk mendapatkan link direct download
...
module.exports = async (sock, m, text, from) => {
    ...
    const originalFilename = mediaMessage.fileName || `file-to-convert.${mediaType === 'image' ? 'jpg' : 'txt'}`;
    ...
    try {
        // --- 4. Upload File ke Host Sementara ---
        await sock.sendMessage(from, { text: "Mengunggah file..." }, { quoted: m });
        const remoteUrl = await uploadFile(fileBuffer, originalFilename);
        console.log("DEBUG: File diunggah ke:", remoteUrl);
        ...
        console.log("DEBUG: Job ID diterima:", jobId);
        if (!jobId) throw new Error("Gagal mendapatkan Job ID dari API.");

        // --- 6. Polling Status Job ---
        let jobStatus;
        let attempts = 0;
        ...
        do {
            ...
            console.log(`DEBUG: Cek ke-${attempts + 1}, Status: ${jobStatus.status.code}`);
            attempts++;
        ...
        if (jobStatus.status.code !== 'completed') {
            console.log("DEBUG: Status akhir job:", jobStatus.status);
        ...
        console.log("DEBUG: URL output PDF:", outputUrl);
        ...
        console.error("PDF Converter process error:", e);
        const errorMessage = e.response?.data?.message || e.message || 'Unknown error';
        await sock.sendMessage(from, { react: { text: '❌', key: m.key } });
        return sock.sendMessage(from, { text: `❌ Terjadi kesalahan: ${errorMessage}` }, { quoted: m });
    }
};
